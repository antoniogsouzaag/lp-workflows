...
type DynamicAnimationOptions = Parameters<typeof motion.span>[0]["transition"];
...
const VerticalCutReveal = forwardRef<VerticalCutRevealRef, TextProps>(
  (
    ...
    const asWordObjects: WordObject[] =
      splitBy === "characters"
    ...
    return (
      <span
        ref={containerRef}
        className={`flex flex-wrap whitespace-pre-wrap ${splitBy === "lines" ? "flex-col" : ""} ${containerClassName}`}
        onClick={onClick}
      >
        <span className="sr-only">{text}</span>
        {asWordObjects.map((wordObj, wordIndex, array) => {
          const prevChars = array.slice(0, wordIndex).reduce((s, w) => s + w.characters.length, 0);
          return (
            <span
              key={wordIndex}
              aria-hidden="true"
              className={`inline-flex overflow-hidden ${wordLevelClassName}`}
            >
              {wordObj.characters.map((char, charIndex) => (
                <span
                  key={charIndex}
                  className={`whitespace-pre-wrap relative ${elementLevelClassName}`}
                >
                  <motion.span
                    custom={prevChars + charIndex}
                    initial="hidden"
